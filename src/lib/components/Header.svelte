<nav class="bg-white">
	<div class="mx-auto max-w-7xl px-4">
		<div class="flex h-16 items-center justify-between">
			<div class="flex items-center space-x-2">
				<svg
					xmlns="http://www.w3.org/2000/svg"
					fill="none"
					viewBox="0 0 24 24"
					stroke-width="1.5"
					stroke="currentColor"
					class="h-7 w-7 text-blue-500"
				>
					<path
						stroke-linecap="round"
						stroke-linejoin="round"
						d="M12 3l8.485 4.243c.184.092.29.283.29.49V19.5a.75.75 0 01-.75.75H4.5a.75.75 0 01-.75-.75V7.733c0-.207.106-.398.29-.49L12 3z"
					/>
					<path stroke-linecap="round" stroke-linejoin="round" d="M9 21V9h6v12" />
				</svg>

				<span class="text-lg font-medium text-black"> Schoolplein ontwerper </span>
			</div>

			<a
				href="/login"
				class="hover:bg-brand-strong inline-block rounded-md bg-black px-3 py-2 !text-white text-white no-underline"
			>
				Inloggen
			</a>
		</div>
	</div>
</nav>
